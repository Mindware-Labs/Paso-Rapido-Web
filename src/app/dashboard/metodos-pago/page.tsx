...
export default function MetodosPagoPage() {
  ...
  const filteredMethods = useMemo(() => {
    return methods.filter((m) => {
      ...
      const matchesBrand =
        filterBrand === "all" ? true : m.brand === filterBrand;
      return matchesSearch && matchesBrand;
    });
  }, [searchTerm, filterBrand]);

  return (
    <div className="min-h-full bg-[#F8F9FB] pb-10">
      <div className="w-full space-y-5 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <button className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-primary/90 transition-all active:scale-95">
        ...
        {/* ── Filtros ── */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-lg border border-gray-100 bg-white p-3 shadow-sm">
                  ...
                  return (
                    <div
                      ...
                      className={`group relative flex items-center justify-between gap-4 px-4 py-3 hover:bg-gray-50/50 transition-colors ${
                        m.isDefault
                          ? "bg-gradient-to-r from-emerald-50/30 to-white"
                          : ""
                      }`}
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span
                          className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-md ${m.brandBg}`}
                        >
                          <CreditCard
                            className={`h-4 w-4 ${m.brandColor}`}
                            strokeWidth={1.75}
                          />
                        </span>
        ...
        <section>
          <h2 className="text-sm font-semibold text-gray-900 mb-3">
            Límites y condiciones de recarga
          </h2>
        ...
        {/* ── Seguridad ── */}
        <div className="flex items-center gap-3 rounded-lg border border-blue-100 bg-gradient-to-r from-blue-50/60 to-white px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-gray-500">
              Tus datos están encriptados. No almacenamos CVV.
            </p>
          </div>
          <Link
